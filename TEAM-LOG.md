...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | *(free)* | — | 14 Aug: final spatial system frozen by Codex — 92% cards, 5.2% larger desktop plate, `82 / 68 / 55px` stems, and enlarged displaced organic washes. Anchors and map geometry unchanged. Card-design phase may proceed independently. |
