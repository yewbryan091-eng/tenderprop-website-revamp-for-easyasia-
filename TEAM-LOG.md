...
|---|---|---|---|
| 14 Aug (eve) | **The plate is CARVED, not decorated — four highland masses in relief.** Titiwangsa down the spine, Banjaran Timur / Tahan, the Kledang flank in Perak, and Gunung Ledang as a single dab in Johor. Each is a blurred stroke pair — dark pushed down-right away from the key light, light pulled up-left toward it — in `soft-light`, sitting BELOW the state dots and the grain so both read through. Every control point is a real place projected through the peninsula's own projection, recovered from the three anchors in `west-malaysia-geometry.ts` (x = 215.08 per degree of longitude from 99.64525E, y = 215.60 per degree of latitude down from 6.708N; all three anchors agree to 5 s.f.). **This is tonal shading, never a drawn feature** — if it ever hardens into something you could trace, it has gone wrong, and it is not a claim about exact ridge lines | Bryan: *"how can we make the map become alive visually?"* The plate was one flat cream field — half the object undifferentiated, and the dotted state borders dividing nothing. It is a stone RELIEF, so the honest answer was to carve it rather than decorate it. First pass at 92/62px strokes read as a smear across the middle; narrowed to 58/40 and broken into four masses, it reads as a range | Bryan asking, Claude proposing |
