...
|---|---|---|---|---|
| 14 Aug (night) | **Section 2's method rows are bespoke inline-SVG editorial illustrations with a ONE-SHOT sequenced entrance.** Envelope+padlock for E-Tender (the wax seal was swapped for a padlock 15 Aug — Bryan wanted the drawing to say "sealed" literally rather than symbolically), gavel+strike block for Owner Auction — thin warm-ink linework, cream fills, one blurred muted wash behind each (#8E2233 / #B58F3E at 0.1); inline so CSS can move flap, seal and gavel as parts. Sequence (retimed 15 Aug so each gesture is actually READABLE): copy → envelope row (150ms) → flap FOLDS shut (330–790ms) → padlock presses (720ms) → auction row (620ms) → gavel held raised at −30° then strikes (1000–1340ms) → impact ticks flash and settle to 0.55 (1300ms) → disclaimer (1500ms). The flap's fold was `scaleY(.88→1)`, a 12% squash nobody could see; it now starts flipped UP about its hinge and passes through zero, which is the real gesture. Only loop anywhere: the LIVE dot pulse (1.6s). ⚠️ **TESTING NOTE: the Claude-in-Chrome tab is `visibilityState: hidden`.** IntersectionObserver and requestAnimationFrame do NOT fire in it — a synthetic IO in that tab never delivered once. Any scroll/observer behaviour must be tested by driving a VISIBLE page over CDP (script kept in the session scratchpad), never through the extension tab, or it will look broken when it is fine and vice versa. **Hovering either drawing replays its own gesture** (16 Aug), driven by the WEB ANIMATIONS API, not a CSS class — decorative only, no cursor change and nothing clickable, and `hover: hover` keeps it off touch where pointerenter fires on tap. Two CSS attempts are recorded in `home-twoways.css` because both look correct and are not: reusing the `tw-flap`/`tw-strike` names does NOTHING (an element already carrying an animation of that name keeps the finished one and merely adopts the new timing), and distinct `-h` keyframes run but firing a stray second gesture ~1s later when the class comes off and the `.tw--play` rule is handed back a 'fresh' animation. **The entrance REPLAYS** (16 Aug): ONE observer, ONE boundary, on `.tw-routes` at rootMargin -8%. Two observers oscillated over the band between them. Watching `.tw` failed differently: it starts 802px down an 801px viewport, so "fully off screen" was only true within a PIXEL of the very top — scroll up to the hero and a sliver stayed, so it never re-armed. The routes sit ~380px lower and clear the viewport on any normal scroll. The -8% is measured: at -20% the trigger line sat 7px above where the routes reach on a normal scroll-in (578 vs 571) and the only thing that ever fired was the 8s guard. Resetting is SILENT — the resting state IS the finished state — so dropping `tw--play` mid-scroll changes nothing on screen. A fresh load already inside the section stays still; the initial measurement is deferred two frames because a `#how-it-works` load has not applied its anchor scroll when effects run. Proven over CDP by scrolling for real: armed → play(running) → finished → re-armed → play(running again) → fresh deep-link stays drawn. **Arm-never-reveal is honoured**: markup rests fully drawn; JS adds `.tw--armed`/`.tw--play` via classList (never React state), skips arming if the section is already on screen, double-gates reduced motion (JS + media query), and an 8s guard forces play if the observer starves — the section cannot ship blank. ⚠️ ChatGPT's proposed support line "your offer stays confidential until closing" was **REJECTED**: offers are presented to the seller immediately (29 Jul ledger) and privacy means hidden from OTHER BUYERS — the shipped line is "Submit privately — your offer is never shown to other buyers." "When the hammer falls, it's sold" not used. LIVE resolved the same night by Bryan's four-point review: the tag moved OUT of the drawing to a brass `● LIVE BIDDING` eyebrow beside the Owner Auction heading (reads as format, not as an event running now); the gavel scene scaled +18% and nudged right/up for equal presence with the envelope; impact ticks cut to two; both support lines rebalanced to equal two-line depth — E-Tender `Name your price privately. Other buyers never see your offer.`, Owner Auction `Bid openly against other buyers on auction day.` | Bryan, from ChatGPT's illustration spec | Bryan |
| 14 Aug | **Section 2's left column is a STORY, not a second explainer.** Kicker is `TWO WAYS TO BUY · DUA CARA` — the bilingual pairing stays, but English now LEADS and the Malay follows (it was `Dua cara · two ways to buy`). Headline is `Across Malaysia, two ways to buy.` — two lines, 56px. The old `From Perlis to Sabah, the same house can be bought two ways` is retired for two reasons: the illustration is peninsula-only, so it named the one place the map deliberately omits; and "the same house" implied a single property runs BOTH routes, which is not the model — different properties, different places, two ways to buy. "Across Malaysia" (not Peninsular) is deliberate: the proposition is national and the listing data carries every state including Sabah and Sarawak — the peninsula-only drawing is an illustration decision, not the limit of the offer. **The two boxed E-Tender / Owner Auction explainers are DELETED.** Section 1 already teaches both routes in full, so repeating them here was redundant, and two bordered cards on the left fighting three property cards on the right made the section read as a dashboard. They are replaced by an unboxed two-row `<dl>`: term in 21px serif, meaning in 14.5px sans, no border, fill or background | Bryan, on ChatGPT's restructure | Bryan |
| 14 Aug | **Section 2's three property cards are drawn in a FLAT overlay, and the connector is drawn WITH the card — not with the map.** A card placed inside the 3D stem chain lies down on the stone: cancelling the camera's `rotateX(60deg) rotateZ(-7deg)` works for the 1.3px stem line, whose width hides the residual perspective skew, but a 152px card shows it immediately. Anchoring a flat card to the 3D stem's measured TIP also failed — they appeared joined at one viewport and visibly detached at the next, which is what Bryan caught. **The fix is that the connector and the card now share one flat coordinate space**, so they cannot drift: `useStemTips()` measures each PEG's rendered centre, and the connector runs from there to the card's foot. Verified 0px at both joins. The 3D `.wm-stem-line` is `display: none`, kept rather than deleted because the pools and pegs were composed against it. Cards are STAGED content written in the component, never live listings — the 13 Aug ledger forbids live listings here — every element is a span and the layer is `pointer-events: none`. All cards now use `dx: 0`; the north and central anchors were moved within their real regions so every straight stem meets its card at bottom centre without overlap | Bryan supplied a reference and asked for cards on the three stems | Bryan |
...
| Owner Auction detail — Bid Information dossier | `src/components/tender/AuctionSinaranDetail.tsx`, scoped Owner Auction dossier rules in `src/styles/tender-detail.css` | *(free)* | — | Completed 14 Aug: auction-day docket + registration cutoff left; Starting bid, live-bidding outcome and Register action right. Desktop and 375px verified; E-Tender page untouched. |
...
| 14 Aug (night) | ~~**The plate is CARVED — four highland masses in relief.**~~ **REVERTED the same night — the plate is CLEAN, do not re-add blurred relief strokes.** The attempt (blurred soft-light stroke pairs for Titiwangsa, Banjaran Timur, Kledang, Ledang) rendered as dark greasy smudges dragged across the stone at real window size. The failure was in the CHECKING, not just the technique: it was judged on screenshots downscaled to ~50%, where heavy blur reads as gentle shading — at 1:1 the same layers read as dirt. Any future terrain must be built from real elevation-classed polygons (or per-state tonal fills), not blurred strokes, and must be verified at 1:1 and zoomed-in crops before calling it done | Bryan, seeing it live: *"wtf is this bro, im dissapointed, remember to always check your work"* | Bryan |
| 14 Aug (eve) | **Section 2's map carries METHOD FLAGS, not property cards.** Each anchor now shows three lines — route (glyph + E-Tender / Owner Auction), town and state, price — on a small cream plaque with a 2.5px route-coloured top edge. No photo, no property name, no countdown, no reserve/starting-bid label. E-Tender keeps burgundy, Owner Auction keeps brass; the brass small-caps line darkens to `#7d5a22` so 8.5px type stays legible on cream. Classes renamed `wm-card*` → `wm-flag*` throughout | A property card is the visual grammar of INVENTORY — it promises a click. Section 2 renders no live listings and nothing in it is clickable, so the card was writing a cheque the section could not cash, which is exactly why an "Illustration only" footnote had to apologise underneath it. Three dense 125px objects also sat on the largest, calmest object on the page and fought it. Bryan: *"i love the idea of the map, i just dont know what to do with the cards — is cards the right idea?"*, then chose flags from three options | Bryan, Claude proposing |
| 14 Aug (eve) | **State separations are DOTTED and MAROON.** `stroke-dasharray: 0.1 4.6` with round caps on both the dark layer (`--burgundy`, 1.6px, opacity 0.72) and the light lower-lip layer beneath it (both must dot in step, or the pair reads as a misregistered second border). Opacity is far above the old 0.28 because a dotted line puts much less ink on the stone, and because maroon read through warm limestone greys out to brown below ~0.6 | Bryan, directly: *"draw out dotted lines for each state seperation within the map, make the dotted line colour maroon"* | Bryan |
| 14 Aug (eve) | **The plate is ~1.26× larger and the section is 854px, not 1066px.** Camera `min(112%, 748px)`; `.tw-inner` columns `0.76fr / 1.24fr`; `.tw-scene` bleeds LEFT (`margin-left: -clamp(0px, 2vw, 30px)`), never right — the plate is centred in its column, so a right-side bleed pushes the coastline further right, which is backwards. Johor's tip is the binding edge and now clears the window by 21px at 1470px. The camera also takes negative `margin-top`/`margin-bottom`: `rotateX(60deg)` means it paints ~61% of the height its layout box reserves, and the surplus was 318px of air above the flags and 186px below the shadow — 47% of the section | Bryan: *"enlarge the map, and i mean enlarge it BIG"*, then *"ok its too big, smaller"*. The 1.59× version clipped 146px of Johor off the right. With the copy beside it, ~1.3× is the ceiling that keeps the whole peninsula on screen — bigger than that needs the copy to move above the plate | Bryan |
| 14 Aug (eve) | **The sidewall has NO per-layer stroke, and is sliced 42 ways with a per-slice flat colour.** Every one of the 21 copies used to carry its own 0.75px non-scaling hairline, so the wall drew 21 dark outlines ~1.8px apart and read as corrugated cardboard — countable along the Perak coast, and Langkawi came out as a stack of pancakes. Only the deepest copy keeps a stroke now, for a crisp foot against the paper. The gradient fill was replaced by a per-slice `color-mix` through the same three `--wm-side-*` stops: an objectBoundingBox gradient on 21 identical boxes ramped over the map's 1334-unit height, so it barely varied across a 68-unit wall. TOTAL DROP is unchanged (15.12 / 68.25) | The stroke was never holding the wall together — consecutive copies overlap by far more than their offset, so the fills already union into one solid mass. Removing it costs nothing | Claude |
| 14 Aug (eve) | **Section 2 photography is retired with the cards**, but the rule that retired two of the three photos stands: a floodlit motorsport grandstand cannot illustrate a RM385k house in Sungai Petani, and a glass-walled city penthouse cannot illustrate a RM465k house in Kuala Lipis. `tenders.ts` already reuses one photo across several towns, so reuse is the house pattern — a mismatched CLASS of building is what breaks the read | Caught on the render, not in the diff | Claude |
| 14 Aug | **Owner Auction says “Bid Information” and “Starting bid”; it does not inherit E-Tender’s Tender Information / reserve-guide / accepted-countered outcome language.** Its left side is one auction-day docket: large day numeral, weekday + month/year + time in one right-hand stack, then registration close + days left as a paired footer. Its right side is starting bid → 3% of successful bid → live-bidding method → auction outcome → Register to bid. `startingBid` is now an EasyAsia contract field required for Owner Auction listings | Bryan corrected the copied detail page over the rendered section. An auction is one scheduled live event and its opening amount is not the E-Tender guide buyers may offer below; using the same labels taught the wrong method | Bryan + Codex |
...
Added the fourth cloud already implied by the map's four-region cloud composition: a flipped,
slightly rotated 0.78-scale puff at SVG `(695, 435)` with 0.82 opacity. The first two rendered
positions sat too close to the central-range cloud and then the Kuala Lumpur placard, so the final
was moved east and north into the user-marked open Pahang lowland. Its existing clipped shadow and
shared cumulus shape keep it fully inland and visually consistent; terrain, rivers, camera,
placards, hotspots and the other three clouds are unchanged. Visually checked at 1440×900,
1728×1050 and 390×844. Focused Prettier and ESLint plus the production build pass. Project-wide
lint still reports the existing repository-wide Prettier backlog (436 problems outside this change).

### 16 Aug 2026 — Codex · Section 2 annotation system finalized

Replaced the mixed full-length diagonal/near-invisible connector behavior with one flat annotation
language: a 30–34px fading hairline leaves each compact ivory placard and stops before its local
watercolor wash, while a 4.4px terminal stays on the exact projected city coordinate. Placard
composition is now independent from geography (`dx +48 / -6 / -112px`, lift `32 / 46 / 44px`),
putting Penang lower, Kuala Lumpur slightly west, and Johor left/down in a descending diagonal.

Placards measure 103–117 × 54px at 1440 (roughly 88% of the prior 117–133 × 62px treatment), with
warmer ivory and a lighter contact shadow. Three displaced low-opacity wash layers now measure
106 / 119 / 120px; Johor uses a softer desaturated rose and no dark centre. Exact WGS84 city inputs
are restored to the generator so regenerated output cannot drift from the hotspots. Rendered QA
passed at 1440×900, 1728×1050 and 390×844 with no card collisions or horizontal overflow. Focused
ESLint, `git diff --check`, and the production build pass; build emits only the existing Nitro
`inlineDynamicImports` warning. Map terrain/camera/geometry, Section 2 left content, Section 1 and
the site shell were untouched.

### 15 Aug 2026 — Codex · Section 2 carved terrain locked

Replaced the former long crest highlight/shadow pairs and open erosion strokes with five broad
authored highland masks. The primary Titiwangsa spine is split north/central/south with overlapping
shoulders; Bintang/Kledang and Timur/Tahan remain secondary. Thirteen branching, varied-width
valleys and tapered tips are cut out before one bounded diffuse-light pass, so ridges and saddles
read from volume rather than pale lines. A separate masked two-scale field supplies low-frequency
landform and restrained erosion. Whole-face warm mottle, mineral grain and isotropic paper tooth
make the plate tactile without turning the quiet eastern and southern plains mountainous.

Compared three rendered treatments, then incorporated two rounds from the independent design
critic: retain the connected five-zone system, sharpen selected valley shoulders, vary channel
widths and taper endpoints. Final browser review passed at 1440×900, 1728×1050 and 375×812 with no
horizontal overflow; a fresh load has no warnings or errors. Focused ESLint, `git diff --check` and
the full production build pass. Only `WestMalaysiaMap.tsx` and `west-malaysia-map.css` changed in
the implementation; coastline, projection, camera, state geometry and every card/stem/wash/left
section asset remain untouched. The terrain zones are geographically grounded illustration, not
an elevation dataset.

### 14 Aug 2026 — Codex · Section 2 spatial system frozen

Took the live local Section 2 state over directly from Claude Code on Bryan's instruction and
completed the final spatial-only pass. The three staged cards now render at a uniform 92% scale
(136px layout width / 125.12px painted width), while the desktop plate grows from 96% / 720px to
101% / 760px — a 5.2% relative increase. Straight centred stems remain exactly attached to their
geographic pegs and card feet at `1.5px`, with restrained opacity raised `0.70 → 0.76`; heights are
`82 / 68 / 55px` for Kedah / Pahang / Johor, down `18% / 15% / 11%` from `100 / 80 / 62px`.

The former soft oval pools are now three different lobed contours per point, enlarged by
`1.34–1.45×`, separated off-centre, displaced with fractal noise and softened at low
`0.085 / 0.105 / 0.11` opacity. They remain neutral per-property connector colours, never method
colours. No card copy, internals, image crop, pills, type, price hierarchy, radii, shadows, left-side
content, other sections or product pages changed. Rendered and measured at 1440×900, the current
1280×720 desktop viewport and 390×844; zero page overflow and zero browser warnings/errors.
Pinned Natural Earth regeneration and the production build pass. Focused lint passes with zero
errors; full-repo lint remains at its existing 381-problem backlog outside this change.
...
### 14 Aug 2026 — Codex · Owner Auction Bid Information dossier rebuilt

Bryan identified the Owner Auction detail as an E-Tender duplicate and started with this dossier.
Three rendered left-side treatments were compared; the event docket won because auction day stays
dominant while registration urgency is subordinate. Final left: flat espresso city wash, auction
day status, large `12`, Saturday / December 2026 / 9:00 AM MYT as one right-hand stack, then
registration closing date paired with days left. The independent design critic agreed that the
large day numeral had earned the weight and diagnosed the date lockup on the **place** axis; moving
Saturday into the right-hand stack removed its collision with the numeral.

Bryan then expanded the pass to the right side: subnav `Bid Information`, `Starting bid`, “Live
bidding begins from this price”, Owner Auction explainer route, and live-auction outcome copy replace
the remaining reserve-guide and accepted/countered E-Tender language. `BACKEND-CONTRACT.md` now
requires `startingBid` for Owner Auction records; the current demo still displays the staged
RM517,000 because the shared data layer is held by Claude and contains no real auction records.

Verified in browser at 1440px and 375px: repeated 50.4/505.2px desktop rails, repeated 24/336px
mobile rails, 0 horizontal overflow, panel 340px on mobile. Targeted ESLint, `git diff --check` and
the production build pass. The E-Tender detail component and its dossier behaviour are untouched.
