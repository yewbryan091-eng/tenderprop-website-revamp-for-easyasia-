...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, the scene block in `TwoWays.tsx`, `src/styles/home-twoways.css`, `src/routes/sandbox.tsx`, `src/styles/sandbox-dua-cara.css`, `design/west-malaysia-*` | **Codex (Claude audit)** | 14 Aug | Active: replace the approximate/dotted geography with a published-vector Peninsular Malaysia plate; map only, no cards, pins, labels or Section 1/left-column changes. SVG/CSS layered extrusion first; desktop visual QA in Chrome. |
...
|---|---|---|---|
| 13 Aug (night) | **Section 2's right side ships as a TWO-LAYER composition: a pre-rendered 3D illustration + HTML overlays.** Pipeline: Figma composition → clean SVG (`design/klang-valley-spline.svg`) → Spline (extrude, tilt, light) → transparent WebP/PNG → Claude Code overlays the 3 property cards, the metric and all type as static HTML. NO text baked into the render; NO Three.js/WebGL/map embeds; still non-interactive per the 13 Aug showcase rule. The `/sandbox` route holds the CSS-3D composition studies that set the blueprint | Bryan chose the approach (via a ChatGPT tooling plan) after CSS-3D studies. HTML overlay keeps text sharp, examples swappable without re-rendering, and lets the metric read the real listing count so it can never go stale | Bryan |
