...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | **Claude Code (Codex supervision)** | 14 Aug | Active: add three independent, coordinate-projected property-location pools and matching staggered stems for Setia Alam, Petaling Jaya and Seri Kembangan. Preserve the approved limestone geography/extrusion; no state/Klang Valley category fill, method colours, labels, metric or property cards. Section 2 left column and Section 1 are locked. |
