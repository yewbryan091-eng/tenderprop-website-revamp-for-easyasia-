...
| 6 Aug | ⚠️ **Nav item reads "Tender", not "E-Tender"** — the ONLY bare "tender" in rendered copy site-wide | Bryan, 6 Aug, a deliberate exception to his own 30 Jul rule (*"all the word tender, must be e-tender"*). **NOT generalised** — every other surface still reads E-Tender. Recorded in `DESIGN-SYSTEM.md` §3c; the H11 guard now means "zero bare tender outside `SiteHeader.tsx`". If the rule is meant to change site-wide, it changes in §3c first | Bryan |
