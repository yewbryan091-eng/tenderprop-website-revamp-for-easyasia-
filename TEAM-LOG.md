...
|---|---|---|---|---|
| 14 Aug | **Section 2's three property cards are drawn in a FLAT overlay, and the connector is drawn WITH the card — not with the map.** A card placed inside the 3D stem chain lies down on the stone: cancelling the camera's `rotateX(60deg) rotateZ(-7deg)` works for the 1.3px stem line, whose width hides the residual perspective skew, but a 152px card shows it immediately. Anchoring a flat card to the 3D stem's measured TIP also failed — they appeared joined at one viewport and visibly detached at the next, which is what Bryan caught. **The fix is that the connector and the card now share one flat coordinate space**, so they cannot drift: `useStemTips()` measures each PEG's rendered centre, and the connector runs from there to the card's foot. Verified 0px at both joins. The 3D `.wm-stem-line` is `display: none`, kept rather than deleted because the pools and pegs were composed against it. Cards are STAGED content written in the component, never live listings — the 13 Aug ledger forbids live listings here — every element is a span and the layer is `pointer-events: none`. The two northern pegs sit ~92px apart, narrower than a card, so `dx` fans those cards off their stems' axis rather than centring them | Bryan supplied a reference and asked for cards on the three stems | Bryan |
| 14 Aug | **The three Section 2 anchors are COMPOSITION-LED, not city centres.** Bryan marked three positions on a render; each coordinate is the inverse of his mark through the map's own `project()`, so the stems land exactly where he put them — SVG `171.91,240.9` · `421.59,522.8` · `764.3,965.99`. **This supersedes the George Town / Kuala Lumpur / Johor Bahru set committed earlier the same day** (`6557f8e`), which sat much closer to the peninsula's extremities. They are named by REGION, not by city, because they are not those cities: they fall ~8km from Sungai Petani, in the Cameron Highlands foothills, and ~22km from Kluang. Naming them after major markets they do not sit on would be the dishonest option, and the section already declares itself "Illustration only". ⚠️ **If these ever need to BE the three major subsale markets, the marks have to move** — George Town / KL / Johor Bahru project to `148,280` · `439,769` · `885,1130` | Bryan marked the positions he wanted and asked for the stems relocated there | Bryan |
...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | **Claude (CLI)** | 14 Aug | Bryan handed this over and told me to abandon the previous session; its in-flight work was committed verbatim first (`6557f8e`) so nothing was lost. Now: the three anchors sit where Bryan MARKED them on a render, not on city centres — see ledger. Pools, pegs, stems, map geometry and surrounding UI untouched. |
