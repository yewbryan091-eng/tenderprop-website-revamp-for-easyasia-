...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `src/styles/home-twoways.css` (`.tw-inner` / `.tw-scene` only), `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | **Codex** | 15 Aug | Refining only the SVG/CSS carved-limestone top surface: geography-led relief, fine erosion engraving, material grain and directional light. Coastline, projection, state lines, camera, scene size, flags, stems, washes and left column are frozen. |
