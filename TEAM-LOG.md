...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `src/styles/home-twoways.css` (`.tw-inner` / `.tw-scene` only), `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | *(free)* | — | 15 Aug (Codex): limestone top-surface refinement complete. Broad diffuse terrain was rejected in screenshot critique as a dusty smudge; final uses restrained masked relief beneath broken geography-led crest highlight/shadow pairs, sparse open erosion cuts, stronger whole-face paper/mineral grain and the same upper-left light. Coastline, projection, states, camera, size, flags, stems, pools and left column unchanged. Verified 1440×900, 1728×1050 and zero overflow at 375×812. |
...
|---|---|---|---|
| 15 Aug | **Section 2 terrain reads through crisp carved structure, not a broad painted cloud.** The real-range envelopes remain the low-contrast relief mask, but their heightfield is now a deterministic two-scale surface (broad fractal landform + 16% finer erosion field) under upper-left diffuse/specular light. Titiwangsa is broken into north/central/south crest segments so it cannot become one stripe; Banjaran Timur/Tahan and Bintang/Kledang stay secondary. Paired warm-recess / ivory-catchlight strokes and 14 open slope cuts provide the normal-distance terrain read, while closed envelope contours stay subordinate at `0.06 / 0.16`. Whole-face mineral bedding, fine paper tooth and slow mottle were strengthened separately. **Rejected during the same pass:** a 0.96-strength diffuse terrain bloom looked like thumb-smudged plaster; a higher-frequency version looked like combed fabric. Final masked relief is `0.66`, paper undulation `0.34`, with no visible Gaussian-blur shadow band. | Bryan asked for roughly 55–70% of the carved reference's texture while explicitly forbidding blurry bands, cloth wrinkles and random noise. Three rendered treatments plus an independent screenshot critique showed that structure, not darker paint, was the correct axis. | Bryan directing, Codex building |
