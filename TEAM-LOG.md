...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | *(free)* | — | Completed 14 Aug: three independent GeoNames-coordinate-projected location pools and matching staggered stems for Setia Alam, Petaling Jaya and Seri Kembangan. Approved limestone geography/extrusion preserved; no category fill, method colours, labels, metric or property cards. Desktop 1440×900 and mobile 390×844 verified with zero horizontal overflow. |
...
|---|---|---|---|
| 14 Aug | **Section 2's first property overlay is three independent geographic points, not a state or Klang Valley category fill.** Setia Alam, Petaling Jaya and Seri Kembangan use GeoNames WGS84 coordinates projected through the existing map generator. Each point has one clipped organic surface pool, a centred neutral peg and a matching upright stem at a deliberately staggered height. Dusty rose, muted terracotta and soft olive-taupe are connector colours only; E-Tender / Owner Auction colours and badges belong to the future cards. No labels, metric or property cards in this phase | Selling method belongs to a property, not an area. Colouring geography by method would falsely imply that an entire place offers only one route; the three-point layer instead establishes accurate attachment points and future-card composition without teaching a category map | Bryan + Codex, Claude builder |
