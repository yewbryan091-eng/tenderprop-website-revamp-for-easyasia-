...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | **Codex** | 14 Aug | Final spatial polish only: card/map scale, stem heights/weight, and organic wash footprint. Bryan explicitly transferred the live local state from Claude Code to Codex. Anchors and map geometry remain frozen. |
