...
| Homepage `/` — **fold / section 1** | `src/routes/index.tsx` (hero block), `src/styles/home.css` | *(free)* | — | 13 Aug: Codex's headline + diagonal product panel, then reworked by Claude on Bryan's direction — panel images removed (see ledger), panel shrunk, days-only countdown, route-naming CTAs. Headline scaled down 104/67px → 80/52px on Bryan's call (see ledger). Panel restyled 13 Aug from Bryan's written critique — 0.74 washes, clipped seam, accent strokes, one-line countdown, coloured CTAs (see ledger). **OPEN BUG, pre-existing, NOT introduced by that work: the `.hp-cue` scroll cue overlaps the stacked panel on mobile** — measured 95px of overlap both before and after, because the cue is positioned against `.hp-hero`'s bottom while the hero's height is driven by its own content, so shrinking the panel moves the cue up by the same amount. Needs a structural fix (reserve space for the cue, or put it in flow below the panel on mobile), not a size tweak. Bryan continues in CLI from here. |
