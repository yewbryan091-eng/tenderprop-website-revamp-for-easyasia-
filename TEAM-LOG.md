...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, the scene block in `TwoWays.tsx`, `src/styles/home-twoways.css`, `src/routes/sandbox.tsx`, `src/styles/sandbox-dua-cara.css`, `design/west-malaysia-*` | *(free)* | — | Completed 14 Aug: pinned Natural Earth Peninsular Malaysia geometry, state-line engraving, 21-layer SVG/CSS limestone extrusion, bevel and grounded shape-aware shadows. No cards, pins, labels, East Malaysia, WebGL, Section 1 or left-column changes. Claude audited the first render; Codex applied the depth/surface/shadow corrections and verified responsive renders in Chrome. |
