...
| Homepage `/` — **section 2 right-side map only** | `src/components/home/WestMalaysiaMap.tsx`, `src/styles/west-malaysia-map.css`, `src/styles/home-twoways.css` (`.tw-inner` / `.tw-scene` only), `scripts/generate-west-malaysia-geometry.mjs`, `src/data/west-malaysia-geometry.ts`, `design/west-malaysia-*` | *(free)* | — | 15 Aug (Codex): limestone top-surface refinement complete. The white crest/highlight strokes and open erosion scratches are gone. Five broad, broken geography-led height masks now form one carved western-central spine, with thirteen branching valley cuts subtracted before matte diffuse lighting; secondary Bintang/Kledang and Timur/Tahan zones remain quieter. Whole-face warm mottle, mineral grain and isotropic paper tooth are independent of elevation. Coastline, projection, state geometry, camera, size, flags, stems, pools and left column unchanged. Verified 1440×900, 1728×1050 and zero overflow at 375×812. |
