...
/* The listings stylesheet too: Similar Owner Auctions renders the SAME PropertyCard as the
...
/* ── THE OWNER AUCTION LISTING PAGE (Bryan, 10 Aug) ────────────────────────────
   The Owner Auction half of Residensi Sinaran. `/tender/residensi-sinaran` and
   `/owner-auction/residensi-sinaran` are now two separate pages for the same property.

   A SEPARATE COMPONENT, copied from `ResidensiSinaranDetail.tsx` rather than a `product`
   branch inside it, for two reasons:

   1. TEAM-LOG §1 claims that file for CODEX — held since 3 Aug, last touched 7 Aug — and
      the repo's law is to tell Bryan rather than edit a held area. Copying sidesteps it
      entirely: the E-Tender page is untouched and cannot regress while Codex rebuilds it.
   2. The products genuinely diverge. Counted before starting: breadcrumb, the closing
      countdown, the deposit basis, the method row, the submit-offer flow, the FAQ, the
      agent block and "Similar Owner Auctions" — 8+ sections. Branching that many through a
      1,578-line component makes both harder to read. Same call, for the same reason, as
      splitting `AuctionPropertyCard` from `PropertyCard`.

   ⚠️ THE COST IS DRIFT. This file and Codex's will diverge the moment either changes.
   When the detail page settles and Codex releases the claim, the property-agnostic
   two-thirds — gallery, specs, location, mortgage calculator — should be extracted and
   shared, leaving only the product blocks in each. Logged in TEAM-LOG.

   Class names and DOM ids are unchanged from the canon so EasyAsia can still lift the
   markup. */
...
/* ── THE AUCTION EVENT, from the one shared object ────────────────────────────
   Not re-derived here. `data/owner-auction.ts` is what the hero and every card read,
   and a second copy of the date on the page those cards link to is precisely how a
...
const BAND: BandItem[] = (
  [
    [
      "Bedrooms",
      SINARAN_TENDER.bedrooms,
      "M3 18v-6h18v6M3 12V7M21 12v-1a3 3 0 0 0-3-3h-4v4M3 18v2M21 18v2",
    ],
    [
      "Bathrooms",
      SINARAN_TENDER.bathrooms,
      "M4 12h16v3a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4v-3ZM6 12V6a2 2 0 0 1 4 0M7 19l-1 2M17 19l1 2",
    ],
    [
      "Built-up area",
      SINARAN_TENDER.builtUp,
      "M4 4h16v16H4zM4 9h5M15 20v-5M9 4v5M15 4v5M4 15h5M15 15h5",
    ],
    ["Storeys", SINARAN_TENDER.storeys, "M4 20h16M6 20V9l6-4 6 4v11M10 20v-5h4v5"],
    [
      "Car parks",
      SINARAN_TENDER.carParks,
      "M5 17h14M4 17v-4l2-5h12l2 5v4M4 17v2h2v-2M18 17v2h2v-2",
    ],
...
const NEARBY: { key: string; label: string; items: NearbyItem[] }[] = [
  {
    ...
    items: [
      {
        name: "Hab Taman Sri Muda",
        kind: "Rapid KL bus hub \u00b7 Route 751",
        km: "1.2 km",
        min: "4 min",
      },
      {
        name: "KESAS Highway",
        kind: "Shah Alam Expressway \u00b7 Sri Muda",
        km: "1.9 km",
        min: "5 min",
      },
      { name: "KTM Shah Alam", kind: "KTM Komuter station", km: "3.8 km", min: "8 min" },
      {
        ...
        kind: "Klang \u2013 Kuala Lumpur trunk road",
        km: "5.1 km",
        min: "9 min",
      },
  ...
  },
  {
    ...
    items: [
      {
        name: "Pasar Moden Sri Muda",
        kind: "Fresh market & daily groceries",
        km: "1.4 km",
        min: "4 min",
      },
      { name: "Lotus\u2019s Shah Alam", kind: "Hypermarket & retail", km: "6.6 km", min: "12 min" },
      {
        ...
        kind: "Shopping, dining & entertainment",
        km: "7.8 km",
        min: "14 min",
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        name: "Klinik Kesihatan Sri Muda",
        kind: "Government health clinic",
        km: "2.1 km",
        min: "6 min",
      },
      {
        name: "Hospital Shah Alam",
        kind: "Government hospital \u00b7 Seksyen 7",
        km: "4.2 km",
        min: "9 min",
      },
      {
        name: "KPJ Selangor Specialist",
        kind: "Private specialist hospital",
        km: "4.8 km",
        min: "9 min",
      },
      {
        name: "Columbia Asia Bukit Rimau",
        kind: "24-hour emergency dept.",
        km: "5.4 km",
        min: "10 min",
      },
    ],
...
const FACILITY_ICON: Record<string, string> = {
  ...
  "24-Hour Security": '<circle cx="10" cy="10" r="6.9"/><path d="M10 5.9V10l2.8 1.7"/>',
  "CCTV Surveillance":
  ...
  Gymnasium: '<path d="M4 7.4v5.2M6.8 5.6v8.8M13.2 5.6v8.8M16 7.4v5.2"/><path d="M6.8 10h6.4"/>',
  "Futsal Court":
  ...
  buy: [
    {
      id: "buy-a",
      date: "DD MMM YYYY",
      comparable: "Nearby comparable A",
    ...
    },
    {
      id: "buy-b",
      date: "DD MMM YYYY",
      comparable: "Nearby comparable B",
      context: "Townhouse · nearby project",
      builtUp: "1,4XX sq ft",
      amount: "RM XXX,XXX",
      evidence: "RM XXX psf",
    },
    {
      id: "buy-c",
      date: "DD MMM YYYY",
      comparable: "Nearby comparable C",
      context: "Townhouse · same locality",
      builtUp: "1,3XX sq ft",
      amount: "RM XXX,XXX",
      evidence: "RM XXX psf",
  ...
  rent: [
    {
      id: "rent-a",
      date: "MMM YYYY",
      comparable: "Nearby rental A",
      context: "Townhouse · tenancy evidence",
      builtUp: "1,4XX sq ft",
      amount: "RM X,XXX / mo",
      evidence: "Verified tenancy",
    },
    {
      id: "rent-b",
      date: "MMM YYYY",
      comparable: "Nearby rental B",
      context: "Townhouse · asking evidence",
      builtUp: "1,4XX sq ft",
      amount: "RM X,XXX / mo",
      evidence: "Asking rent",
    },
    {
      id: "rent-c",
      date: "MMM YYYY",
      comparable: "Nearby rental C",
      context: "Townhouse · tenancy evidence",
      builtUp: "1,3XX sq ft",
      amount: "RM X,XXX / mo",
      evidence: "Verified tenancy",
...
export function AuctionSinaranDetail() {
  ...
  useEffect(() => {
    try {
      ...
      /* private mode, or corrupt value — default to not saved */
    }
  }, []);
  const toggleSave = useCallback(() => {
    setIsSaved((was) => {
      ...
      try {
        const list = JSON.parse(localStorage.getItem(SAVE_KEY) || "[]") as string[];
        const next = now
          ? [...new Set([...list, LISTING_ID])]
          : list.filter((x) => x !== LISTING_ID);
        localStorage.setItem(SAVE_KEY, JSON.stringify(next));
      } catch {
        /* ignore — the toggle still reflects intent for this session */
      }
      return now;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    import("qrcode")
      ...
      .then((svg) => {
        if (alive) setWaQr(svg);
      })
      .catch(() => {
        /* no QR is fine — the two buttons above it still work */
      });
    return () => {
      alive = false;
    };
  ...
  const step = useCallback(
    (delta: number) =>
      setLightbox((i) => (i === null ? i : (i + delta + PHOTO_COUNT) % PHOTO_COUNT)),
    [PHOTO_COUNT],
  );
  ...
  const closeViewer = useCallback((at: number) => {
    setActive(at);
    setLightbox(null);
  }, []);
  ...
  useEffect(() => {
    const calc = () => {
      ...
      /* ⚠️ REGISTRATION, not the tender close. The kicker above this timer reads
         "Registration closes in", so it must measure `REGISTRATION_CLOSE_MS` — the same
         instant the auction cards count to. Leaving the tender helpers here would have
         counted to a deadline this page no longer shows, and the two would round to the
         same number often enough to hide it. */
      const ms = remainingMsUntil(REGISTRATION_CLOSE_MS);
      const days = daysUntil(REGISTRATION_CLOSE_MS);
      const finalDay = isFinalDayUntil(REGISTRATION_CLOSE_MS);
      setDaysLabel(
        ms <= 0 ? "Closed" : `${days.toLocaleString("en-MY")} ${days === 1 ? "day" : "days"} left`,
      );
  ...
  const mortgagePriceValid =
    mortgagePrice.trim() !== "" &&
    Number.isFinite(mortgagePriceValue) &&
    mortgagePriceValue > 0 &&
    mortgagePriceValue <= 100_000_000;
  const mortgageDownValid =
    mortgageDown.trim() !== "" &&
    Number.isFinite(mortgageDownValue) &&
    mortgageDownValue >= 0 &&
    mortgageDownValue <= 90;
  const mortgageYearsValid =
    Number.isFinite(mortgageYearsValue) && mortgageYearsValue >= 5 && mortgageYearsValue <= 35;
  const mortgageRateValid =
    mortgageRate.trim() !== "" &&
    Number.isFinite(mortgageRateValue) &&
    mortgageRateValue >= 0 &&
    mortgageRateValue <= 20;
  const mortgageValid =
    mortgagePriceValid && mortgageDownValid && mortgageYearsValid && mortgageRateValid;
  ...
  const mortgageMonthly =
    !mortgageValid || mortgageMonths <= 0
      ? 0
  ...
  const mortgageMoney = (amount: number) =>
    ...
    <div className="tp-detail">
      <main>
        ...
        <section className="overview" id="overview">
          <div className="wrap wrap-wide">
            <div className="crumbs">
              <a href="#">Home</a> / <a href="/owner-auction">Owner Auction</a> / Residensi Sinaran
            </div>
            <div className="ovhead">
              <div className="ovtitle">
                ...
                <p className="ovstatus">
                  <span className="status">
                    ...
                    Going under Owner Auction
                  </span>
                  <span className="ovcloses">
                    Closes {TENDER_CLOSE_LABEL}
                    {daysLabel ? " · " + daysLabel : ""}
                  </span>
                </p>
                ...
                <p className="addr">
                  ...
                  Taman Sri Muda, Shah Alam, Selangor &middot; {TYPE_LABEL}
                </p>
              </div>
              <div className="ovside">
                ...
                <div className="ovprice">
                  <div className="v num">RM517,000</div>
                  <div className="s">
                    The Reserve Price &mdash; a guide, you choose what to offer
                  </div>
                </div>
                ...
                <div className="actions">
                  ...
                  >
                    <svg
                      ...
                      aria-hidden="true"
                    >
                      <path d="M12 20.5l-1.4-1.3C5.4 14.5 2 11.4 2 7.6 2 4.9 4.1 3 6.7 3c1.5 0 2.9.7 3.8 1.8L12 6.2l1.5-1.4C14.4 3.7 15.8 3 17.3 3 19.9 3 22 4.9 22 7.6c0 3.8-3.4 6.9-8.6 11.6L12 20.5z" />
                    </svg>
                  ...
                  <a className="btn red" href="#tender">
                    Register to bid
                  </a>
                </div>
              </div>
            ...
            <div className="gallery">
              ...
              >
                <img
                  id="stage-img"
                  src={SINARAN_PHOTOS[active]}
                  alt={`Residensi Sinaran — photo ${active + 1}`}
                />
                <span className="stagecount" id="stagecount">
                  {active + 1} / {PHOTO_COUNT}
                </span>
              ...
              <div className="mediarow">
                {MEDIA.aerialFrom && (
                  <button
                    type="button"
                    className="mediabtn"
                    onClick={() => setActive(MEDIA.aerialFrom! - 1)}
                  >
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
        ...
        <nav className="subnav" id="subnav" aria-label="Section navigation">
          <div className="wrap">
            <div className="row">
              ...
              <a href="#facilities">Facilities</a> <a href="#location">Location</a>{" "}
              <a href="#history">Price History</a> <a href="#agent">Agent</a>{" "}
              <a href="#mortgage">Mortgage</a> <a href="#faq">FAQ</a>
            </div>
          </div>
        </nav>
              ...
              <div className="v1-grid">
                ...
                <div className="v1-main" id="tender-action-panel">
                  ...
                  <div className="v1-terms">
                    <div>
                      {/* ⚠️ NO FIGURE HERE, DELIBERATELY. The E-Tender page prints
                          `{DEPOSIT}` — 3% of the RESERVE, which is knowable up front. An
                          Owner Auction deposit is 3% of the BIDDING price (TEAM-LOG, the
                          standing deposit trap), and nobody knows the bidding price before
                          the room sets it. Printing the tender figure here would state a
                          number that is wrong by construction, so the page states the RULE
                          and leaves the amount to the bid. */}
                      <span className="lbl">Auction deposit</span>
                      <b className="dep-amt">3% of your bid</b>
                      <span className="sub">Payable on a successful bid</span>
                      ...
                      <span className="v1-dep-note">
                        ...
                        returned in full if you are not the successful bidder.
                      </span>
                    </div>
                    <div>
            ...
            <div className="blkcard dcard">
              ...
              <div className="pd-pricing">
                <div>
                  ...
                  <b className="pd-psf">
                    {PSF ? `RM${PSF.toLocaleString("en-MY")}` : "\u2014"}
                    <i>psf</i>
                  </b>
              ...
              <section className="pd-ask">
                ...
                <span className="pd-ask-mark" aria-hidden="true">
                  ?
                </span>
                <div className="pd-ask-main">
                  <h3 className="pd-ask-title">Still have questions about this property?</h3>
                  <p className="pd-ask-lede">
                    If the details above don&rsquo;t answer everything, feel free to ask the
                    appointed agent anything &mdash; from how the auction works to
        ...
        <section className="blk band-card" id="about">
          <div className="wrap">
            <div className="blkcard about">
              <h2 className="sec-title">
                About <span>Residensi Sinaran</span>
              </h2>
        ...
        <section className="blk band-card" id="selling">
          <div className="wrap">
            <div className="blkcard">
              <h2 className="sec-title">
                Selling <span>Points</span>
              </h2>
              <div className="spgrid">
                <div className="sp-item">
                  <h3>Completed Homes, Ready to View</h3>
                  <p>Inspect the actual home and its surroundings before you commit.</p>
                </div>
                <div className="sp-item">
                  <h3>Low-Density Community of 62 Homes</h3>
                  <p>Just 62 three-storey stratified townhouses in total.</p>
                </div>
                <div className="sp-item">
                  <h3>Three-Storey Townhouse Living</h3>
                  <p>Multi-level layouts separate shared and private family space.</p>
                </div>
                <div className="sp-item">
                  <h3>Gated and Guarded Environment</h3>
                  <p>A controlled environment for greater privacy and peace of mind.</p>
                </div>
                <div className="sp-item">
                  <h3>Two Parking Bays per Home</h3>
                  <p>Each residence includes two parking bays.</p>
                </div>
                <div className="sp-item">
                  <h3>Established Taman Sri Muda Location</h3>
                  <p>A mature Shah Alam neighbourhood with schools, shops and healthcare nearby.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="blk band-card" id="area">
          <div className="wrap">
            <div className="blkcard">
              ...
              <p className="sec-note">Approximate driving distance and time from {LISTING_NAME}.</p>
              <div className="amen">
                {NEARBY.map((cat) => (
                  <div className="amencol" key={cat.key}>
                    <h3>
                      <svg
                        ...
                        strokeWidth="2"
                        aria-hidden="true"
                        dangerouslySetInnerHTML={{ __html: NEARBY_ICON[cat.key] }}
                      />
                      {cat.label}
                      ...
                      <span className="amenkey">
                        <span>Distance</span>
                        <span>&middot; Drive</span>
                      </span>
                    </h3>
                    {cat.items.map((it) => (
          ...
          <div className="wrap">
            <div className="blkcard">
              ...
              <div className="mapbox">
                <iframe
                  title="Map — Taman Sri Muda, Shah Alam"
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  src={`https://www.google.com/maps?q=${MAP_EMBED_Q}&output=embed`}
                ></iframe>
              </div>
            </div>
          </div>
            ...
            <div className="blkcard price-history">
              <div className="ph-heading">
                <div>
                  ...
                  <p className="sec-note">
                    Comparable sale price transaction and rental evidence for similar properties
                    near {LISTING_NAME}.
                  </p>
                </div>
              ...
              >
                <div className="ph-ledger-head">
                  <div>
                    <strong>
                      {priceHistoryMode === "buy"
                        ? "Comparable sale records"
                        : "Comparable rental records"}
                    </strong>
                ...
                <div className="ph-table-wrap">
                  <table className="ph-table">
                    <caption className="ph-sr">
                      {priceHistoryMode === "buy"
                        ? "Comparable sale transaction layout"
                        : "Comparable rental evidence layout"}
                    </caption>
                    <thead>
                      <tr>
                        ...
                        <th scope="col" className="ph-amount-head">
                          {priceHistoryMode === "buy" ? "Transacted price" : "Monthly rent"}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {PRICE_HISTORY_PREVIEW[priceHistoryMode].map((row) => (
                        <tr key={row.id}>
                          <td className="ph-date num">
                            <span>{row.date}</span>
                          </td>
                          <td className="ph-comparable">
                            <strong>{row.comparable}</strong>
                            <span>{row.context}</span>
                          </td>
                          <td className="ph-built num">
                            <span>{row.builtUp}</span>
                          </td>
                          <td className="ph-amount num">
            ...
            <div className="blkcard">
              ...
              <div className="agentcard">
                ...
                <div className="ag-who">
                  ...
                  <span className="ag-firm">
                    {AGENT.firm}
                    {AGENT.ssmNo ? ` (${AGENT.ssmNo})` : ""}
                  </span>
                  <span className="ag-reg">
                    {AGENT.reaNo ? `REA ${AGENT.reaNo}` : "REA registration not stated"}
                    {" \u00b7 "}
                    {AGENT.eNo
                      ? `Registered estate agency ${AGENT.eNo}`
                      : "agency registration not stated"}
                  </span>
                </div>
                ...
                <div className="ag-cta">
                  ...
                  <p className="ag-quote">
                    &ldquo;Call me Stephen. Ask what you like about {LISTING_NAME} &mdash; before or
                    after auction day. Either way I&rsquo;m the one who sees it
                    through.&rdquo;
                  </p>
                  ...
                  <div className="ag-btns">
                    ...
                    <a className="btn ag-call" href={`tel:${AGENT_TEL}`}>
                      <svg
                        ...
                        aria-hidden="true"
                      >
                        <path d="M6.3 3.5h3l1.5 3.8-1.9 1.2a11.5 11.5 0 0 0 5.6 5.6l1.2-1.9 3.8 1.5v3a1.8 1.8 0 0 1-2 1.8A15.8 15.8 0 0 1 4.5 5.5a1.8 1.8 0 0 1 1.8-2z" />
                      </svg>
                      {AGENT.phone}
                    </a>
                  </div>
                  ...
                  {waQr && (
                    <div className="ag-qr">
                      <button
                        type="button"
                        className="ag-qr-plate"
                        onClick={() => setQrOpen(true)}
                        aria-label="Enlarge the WhatsApp QR code"
                      >
                      ...
                      <span className="ag-qr-t">
                        Scan to open this chat on your phone
                        {/* Names the affordance. A QR looks like an image, not a control, so
                            without this nobody discovers that it enlarges — and at 86px it is
                            genuinely hard to scan from across a desk. */}
                        <button
                          type="button"
                          className="ag-qr-zoom"
                          onClick={() => setQrOpen(true)}
                        >
                          Click to enlarge
                        </button>
                  ...
                  {qrOpen && waQr && (
                    ...
                    >
                      {/* stopPropagation so a click on the code itself does not dismiss it —
                          people tap the thing they are trying to scan. */}
                      <div className="qrmodal-box" onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          className="qrmodal-x"
                          onClick={() => setQrOpen(false)}
                        ...
                        <span className="qrmodal-code" aria-hidden="true">
                          <span className="ag-qr-img" dangerouslySetInnerHTML={{ __html: waQr }} />
                          <span className="ag-qr-mark">
                            <svg viewBox="0 0 24 24" fill="currentColor" focusable="false">
                              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z" />
                        ...
                        <b className="qrmodal-t">Scan with your phone camera</b>
                        <span className="qrmodal-s">
                          Opens a WhatsApp chat with {AGENT.name} &middot; {AGENT.phone}
                        </span>
                      </div>
                    </div>
                  )}
        ...
        <section className="blk band-card" id="mortgage">
          <div className="wrap">
            ...
            <div className="mortcard">
              <form className="mc-form" onSubmit={(event) => event.preventDefault()} noValidate>
                <div className="mc-fields">
                  <div className="mc-field mc-span2">
                    ...
                    {!mortgagePriceValid && (
                      <span className="mc-error" id="c-price-error">
                        Enter a price above RM0.
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mc-field">
                    ...
                    {!mortgageDownValid && (
                      <span className="mc-error" id="c-down-error">
                        Use 0% to 90%.
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mc-field">
                    ...
                    {!mortgageRateValid && (
                      <span className="mc-error" id="c-rate-error">
                        Use 0% to 20%.
                      </span>
                    )}
                  </div>
              ...
              <div className="mc-out">
                ...
                <dl className="mc-rows">
                  <div>
                    <dt>Loan amount</dt>
                    <dd>{mortgageMoney(mortgageLoan)}</dd>
                  </div>
                </dl>
              </div>
            </div>
            ...
            <p className="mc-note">
              Estimate only. Actual financing depends on bank approval, valuation, loan margin and
            ...
            <div className="faqcard">
              <div className="faq-item">
                ...
                <div className="faq-body">
                  <p>
                    The reserve price (RM517,000) is the minimum the seller will consider. Bids
                    below it will not be selected.
                  </p>
                </div>
              </div>
              <div className="faq-item">
                ...
                <div className="faq-body">
                  <p>
                    Owner Auction is live bidding on a scheduled date. Bids are called openly
                    against the other registered bidders, so the price is set in the room rather
                    than by a sealed offer.
                  </p>
                </div>
              </div>
              <div className="faq-item">
                ...
                <div className="faq-body">
                  <p>
                    The auction is held on <b>{AUCTION_DATE_LABEL}</b> at {AUCTION_TIME_LABEL}.
                    Register through your member account by <b>{REGISTRATION_LABEL}</b> —
              ...
              <div className="faq-item">
                ...
                <div className="faq-body">
                  <p>
                    Yes. Deposits are held by the registered estate agency as stakeholder. If the
                    seller does not accept your offer — including after any negotiation — the agent
                    tells you and returns your deposit in full immediately. There is no waiting
                    period: once all three parties know the sale will not proceed, there is nothing
                    left to wait for.
                  </p>
                </div>
              </div>
        ...
        <section className="blk band-card" id="similar">
          <div className="wrap">
            <h2 className="sec-title">
              Similar <span>Owner Auctions</span>
            </h2>
      ...
      <div className="bidbar" id="bidbar" aria-hidden="true">
        <div className="wrap in">
          <div className="identity">
            <span className="nm">Residensi Sinaran</span>
            <span className="meta">
              Auction {AUCTION_DATE_LABEL} · registration closes {REGISTRATION_LABEL}
            </span>
          </div>
          <div className="pr">
            <div className="k">Reserve price</div>
            <div className="v num">RM517,000</div>
            <span className="closes">Closes {TENDER_CLOSE_LABEL}</span>
          </div>
          <a className="btn red" href="#tender">
            Register to bid
          </a>
      ...
      {lightbox !== null && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) closeViewer(lightbox);
          }}
        >
          <button
            type="button"
          ...
          <img
            id="imgmodal-img"
            src={SINARAN_PHOTOS[lightbox]}
            alt={`Residensi Sinaran photo ${lightbox + 1}`}
          />
          <button
            type="button"
